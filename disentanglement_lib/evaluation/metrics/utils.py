...
def normalize_data(data, mean=None, stddev=None):
    ...
    return np.nan_to_num((data - mean[:, np.newaxis]) / stddev[:, np.newaxis]), mean, stddev
...
def logistic_regression_cv():
    ...
    return LogisticRegressionCV(
        Cs=10,
        cv=KFold(n_splits=5),
        # default value for multi_class changed in the newer version of scikit-learn, which leads to code failing
        multi_class='ovr',
    )
