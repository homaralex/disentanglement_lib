    ...
    with hub.eval_function_for_module(module_path) as f:
        ...
        def _gaussian_encoder(x):
            ...
            while num_outputs < x.shape[0]:
                ...
                output_batch = f(dict(images=inputs), signature="gaussian_encoder", as_dict=True)
                num_outputs += batch_size
                output.append(output_batch)

            output = {key: np.concatenate(np.array(batch[key]) for batch in output) for key in output[0]}
